...
function parseData(obj) {
  ...
  const daysArr = fiveDaysArr.map((day) =>
  ...
  const daysForGalaArr = fiveDaysArr.map((day) =>
    moment(day.dt * 1000).format("D")
  );
  const MaxTempFromFiveDaysArr = daysForGalaArr.map((DAY) => {
    const maxArray = dataArray
      .filter((day) => moment(day.dt * 1000).format("D") == DAY)
  ...
  daysArr.forEach((el, i) => {
    fiveDaysData[i].day = el;
    fiveDaysData[i].weather = obj.list[i].weather[0].main;
  });
  datesArr.forEach((el, i) => {
    fiveDaysData[i].date = el;
  });
  iconsArr.forEach((el, i) => {
    fiveDaysData[i].icon = el;
  });
  MinTempFromFiveDaysArr.forEach((el, i) => {
    fiveDaysData[i].min = el;
  });
  MaxTempFromFiveDaysArr.forEach((el, i) => {
    fiveDaysData[i].max = el;
  });
  daysForGalaArr.forEach((el, i) => {
    fiveDaysData[i].dayForGala = el;
  });
